...
class Vfs::Rump_file_system : public File_system
{
	...
	public:
		...
		: _lock(lock)
		{
			...
			PLOG("%s file system mounted", fs_type.string());
		}
...
class Rump_factory : public Vfs::File_system_factory
{
	private:
		...
		class Sync : public Hard_context, public Genode::Thread_deprecated<1024*sizeof(Genode::addr_t)>
		{
			private:

				enum {
					THREAD_WEIGHT =
						Genode::Cpu_session::Weight::DEFAULT_WEIGHT / 4
				};

				Timer::Connection       timer;
				Genode::Signal_receiver sig_rec;
				Genode::Signal_context  timer_ctx;
				Genode::Lock            lock;
			...
			protected:
				...
				void entry()
				{
					/* make this thread an explicit rump thread */
					rump_pub_lwproc_newlwp(0);

					enum { TEN_SEC = 10*1000*1000 };
					timer.trigger_periodic(TEN_SEC);

					Genode::Signal_context *ctx;
					do {
						ctx = sig_rec.wait_for_signal().context();
						lock.lock();
						_rump_sync();
						lock.unlock();
					} while (ctx == &timer_ctx);

					/* release the thread when signaled by the destructor */
				...
				public:
					...
					:
						/* XXX: hardcoding the cookie is bad */
						Hard_context(3),
						Thread_deprecated(THREAD_WEIGHT, "vfs_rump_sync"),
						lock(lock)
					{
						timer.sigh(sig_rec.manage(&timer_ctx));
					}

					~Sync()
					{
						Genode::Signal_context ctx;
						Genode::Signal_transmitter(sig_rec.manage(&ctx)).submit();
						join();
						sig_rec.dissolve(&ctx);
						sig_rec.dissolve(&timer_ctx);
					}
